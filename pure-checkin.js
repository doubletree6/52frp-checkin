 ...
 *
 * 使用方法：
 *   FRP_USERNAME=your_username FRP_PASSWORD=your_password node pure-checkin.js
 *
 * 或配置 .env 文件：
 *   cp .env.example .env
 *   # 编辑 .env 填入账号密码
 *   node pure-checkin.js
 */

const { pureBrowserCheckIn } = require('./src/browser-pure');
...
async function main() {
  ...
  if (!username || !password) {
    ...
    console.error('  FRP_USERNAME=your_username FRP_PASSWORD=your_password node pure-checkin.js');
    console.error('');
    console.error('方式2: .env 文件');
    console.error('  cp .env.example .env');
    console.error('  # 编辑 .env 填入账号密码');
    console.error('  node pure-checkin.js');
    process.exit(1);
  }
