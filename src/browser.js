...
async function detectServerErrorPage(page) {
  ...
  const match = normalized.match(/Sorry, there was an error on the server[^]*/i);
  if (match) {
...
async function pureBrowserCheckIn({
  ...
  try {
    ...
    // （9/20 即此原因失败）。仍在登录页时重新提交，最多重试 2 次。
    for (let attempt = 1; attempt <= 2 && !loginSuccess; attempt++) {
      ...
      const reclamp = await clickLoginButton(page);
      if (!reclamp.clicked) {
