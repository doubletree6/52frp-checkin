...
- 定时运行（两个时段）
  - **主跑**：名义每天北京时间 **11:15**（cron `15 3 * * *`，UTC）
  - **错峰补跑**：名义每天北京时间 **18:00**（cron `0 10 * * *`，UTC）
  - 补跑只在**当天主跑没有成功**时才真正签到；主跑成功则跳过，不会重复签到、也不会重复通知
  - 说明：GitHub 对 `schedule` 有调度延迟，实测主跑常在北京 15:40–16:50 才落地，
    所以补跑时段放在其后。签到幂等（已签到按成功处理），因此补跑不会造成重复签到。
